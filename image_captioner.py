...
class ImageCaptioner:
    ...
    def infer(self, input_image_filepath, prompt, save_results):
        ...
        inputs = self.processor(
            ...
            "cuda", torch.float16
        )  # Move inputs to GPU with float16 precision
        ...
        generated_ids = self.model.generate(
            ...
            pixel_values=inputs[
                "pixel_values"
            ].cuda(),  # Convert image pixel values to CUDA
        ...
        parsed_answer = self.processor.post_process_generation(
            ...
            image_size=(
                image.width,
                image.height,
            ),  # Original image dimensions for scaling output
        )[prompt]
        ...
        if save_results:
            ...
            self.save_image(result_image=annotator.result(), path="output.jpg")
    ...
    def read_image(self, filename):
        ...
        if filename is not None and os.path.isfile(filename):
            ...
            if not os.path.isfile(image_path):
                ...
                safe_download(
                    f"https://github.com/ultralytics/assets/releases/download/v0.0.0/{image_name}",
                    image_path,
                )

        # Read the image using OpenCV and convert it into the PIL format
        return Image.fromarray(
            cv2.cvtColor(cv2.imread(f"{image_path}"), cv2.COLOR_BGR2RGB)
        )
